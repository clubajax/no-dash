(function (root, factory) {
    ...
    function isDef(item) {
        // is defined, including: 0, false and ''
        return item !== null && item !== undefined && !Number.isNaN(item);
    }
    
    function isValue(item) {
        // isDef, but empty string does not count
        return item !== null && item !== undefined && item !== '' && !Number.isNaN(item);
    }
    ...
    function isEmpty(item) {
        ...
        // Note, this could theorectically be a false flag on an array of zeros
        if (Array.isArray(item) && (!item.length || item.every((m) => isEmpty(m)))) {
            return true;
        }
